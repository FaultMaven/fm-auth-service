...
class TestTokenTampering:
    ...
    @pytest.mark.asyncio
    async def test_modified_claims_rejected(
        self,
        client: AsyncClient,
        admin_access_token: str
    ):
...
class TestPasswordSecurity:
    ...
    @pytest.mark.asyncio
    async def test_login_error_doesnt_reveal_user_existence(
        self,
        client: AsyncClient
    ):
...
class TestInputValidation:
    ...
    async def test_sql_injection_protection(
        ...
        response = await client.post(
            "/api/v1/enterprise/auth/login",
            json={
                "email": "admin@testorg.com' OR '1'='1",
                "password": "anything"
            }
        )
    ...
    @pytest.mark.asyncio
    async def test_xss_protection_in_error_messages(
        self,
        client: AsyncClient
    ):
